    ...
    io.on("connection", async (socket) => {
      ...
      socket.on("enter", async (user,password) => {
        ...
        try {
          ...
          socket.emit("update-user-list",account[0].contacts,accounts.map((u)=>u.account),validPassword)
        } catch (error) {
      ...
      socket.on("send-message", async (msg) => {
        ...
        if(!account[0].contacts.includes(msg.from)){
          ...
          socket.to(onlineUsersList.get(msg.to)).emit("update-user-list",account[0].contacts,null,true)
        }
      ...
      socket.on("get-messages",async (to) => {
        ...
        socket.emit("get-messages", messages.filter((e) => e.from == socket.user && e.to == to || e.from == to && e.to == socket.user));
...
app.post("/register", async (req, res) => {
  ...
  try{
    ...
    if (found) {
      ...
      newUser.save();
      res.status(200).json({ success: true, message: "Registered and connected" });
    }
  }catch (error) {
